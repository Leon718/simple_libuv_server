...
bool TCPServer::bind(const char* ip, int port)
{
    ...
    LOG(INFO) << "server bind ip=" << ip << ", port=" << port;
    return true;
}
...
bool TCPServer::listen(int backlog)
{
    ...
    LOG(INFO) << "server Start listen. Runing.......";
...
void TCPServer::StartThread(void* arg)
{
    ...
    void AfterRecv(uv_stream_t* handle, ssize_t nread, const uv_buf_t* buf)
    {
        ...
        if (nread < 0) {/* Error or EOF */
            if (nread == UV_EOF) {
                ...
                LOG(INFO)<<"client(" << theclass->clientid << ")eof";
            } else if (nread == UV_ECONNRESET) {
                fprintf(stdout, "client(%d)conn reset\n", theclass->clientid);
                LOG(INFO)<<"client(" << theclass->clientid << ")conn reset";
            } else {
                fprintf(stdout, "%s\n", GetUVError(nread).c_str());
                LOG(INFO)<<"client(" << theclass->clientid << ")：" << GetUVError(nread);
            }
    ...
    void AfterSend(uv_write_t* req, int status)
    {
        ...
        if (status < 0) {
            LOG(ERROR)<<"send data error:" << GetUVError(status);
...
void TCPServer::AcceptConnection(uv_stream_t* server, int status)
{
    ...
    LOG(INFO)<<"new client id=" << clientid;
    return;
}
...
void TCPServer::SubClientClosed(int clientid, void* userdata)
{
    ...
    if (itfind != theclass->clients_list_.end()) {
        ...
        LOG(INFO)<<"delete client:" << itfind->first;
...
{
    if (senddata.empty()) {
        LOG(INFO)<<"broadcast data is empty.";
        return true;
    }
    uv_mutex_lock(&mutex_clients_);
...
bool TCPServer::sendinl(const std::string& senddata, TcpClientCtx* client)
{
    ...
    if (iret) {
        ...
        LOG(ERROR)<<"client(" << client << ") send error:" << GetUVError(iret).c_str();
