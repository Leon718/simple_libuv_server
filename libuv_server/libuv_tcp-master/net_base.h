...
inline uint64_t _htonll(uint64_t num)
{
    int i = 0x1;
    uint64_t retval;
    if(*(char *)&i == 0x1) {
...
inline uint64_t _ntohll(uint64_t num)
{
    int i = 0x1;
    uint64_t retval;
    if(*(char *)&i == 0x1) {
...
{
    uint64_t network_byteorder=Phata::_htonll(intnum);//转换为网络字节序
...
inline bool CharToInt64(const unsigned char* charnum, uint64_t& intnum)
{
    ...
    intnum=Phata::_ntohll(intnum);//转换为网络字节序
    return true;
}
